...
outfile = f"{out_path}/BACoN_pmt_analysis_heights_{file_name}"
...
def height_of_peaks2(waveforms, peaks):
    all_heights = np.array(list(map(pf.peak_height, waveforms, peaks)), dtype=object)
    return all_heights
...
height_peaks                = height_of_peaks2(zswfs, idx_peaks_max) ## Heights not concatenated
